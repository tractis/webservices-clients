...
class ContentStorageServiceClient
  ...
  def initialize
    @@location ||= '/lta'
    endpoint_url = "http://#{BACKEND[:HOST]}#{@@location}".to_s #TODO change it to point to localhost!
  ...
  def list
	response = @endpoint.list(ContentListRequest.new)
	response.contentIdentifier
  end

  def delete(identifiers)
	request = ContentRemoveRequest.new
	request.contentIdentifier = identifiers
	response = @endpoint.remove(request)
...
require 'content_storage_service_client';BACKEND = {:HOST => 'trunk.tractis.com:8080/content_storage', :PORT => 8080};cs = ContentStorageServiceClient.new
#require 'ruby-debug';debugger
l = Time.new
#file = File.read("/home/dave/asset.bin")
file = File.read("/home/dave/store_index.html")
#file = "AAA"
res = cs.store(file,{"type"=>'picture'})
